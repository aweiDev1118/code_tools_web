...
const copyAllInfo = () => {
  ...
  const info = `${t('tool.ip-lookup.ipAddress')}: ${ipInfo.value.ip}
${t('tool.ip-lookup.country')}: ${ipInfo.value.country} (${ipInfo.value.countryCode})
${t('tool.ip-lookup.province')}: ${ipInfo.value.regionName}
${t('tool.ip-lookup.city')}: ${ipInfo.value.city}
${t('tool.ip-lookup.postalCode')}: ${ipInfo.value.zip}
${t('tool.ip-lookup.coordinates')}: ${ipInfo.value.lat}, ${ipInfo.value.lon}
${t('tool.ip-lookup.timezone')}: ${ipInfo.value.timezone}
${t('tool.ip-lookup.isp')}: ${ipInfo.value.isp}
AS: ${ipInfo.value.as}`
...
<template>
  <div class="max-w-4xl mx-auto">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ t('tool.ip-lookup.name') }}</h1>
      <p class="text-gray-500">{{ t('tool.ip-lookup.subtitle') }}</p>
    </div>
    ...
    <el-card class="mb-4">
      <div class="flex flex-wrap gap-3 items-center">
        <el-input
          v-model="queryIp"
          :placeholder="t('tool.ip-lookup.queryPlaceholder')"
        ...
        <el-button type="primary" @click="queryCustomIp" :loading="loading">
          {{ t('tool.ip-lookup.query') }}
        </el-button>
        <el-button @click="refresh" :loading="loading">
          <el-icon class="mr-1"><Refresh /></el-icon>
          {{ t('tool.ip-lookup.getMyIp') }}
        </el-button>
      </div>
    </el-card>
    ...
    <el-card v-if="loading" class="mb-4">
      <div class="loading-display">
        ...
        <p class="loading-text">{{ t('tool.ip-lookup.fetchingIp') }}</p>
      </div>
    </el-card>
        ...
        <el-card>
          <template #header>
            <div class="card-header">
              ...
              <span>{{ t('tool.ip-lookup.geoLocation') }}</span>
            </div>
          </template>
          <div class="info-grid">
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.country') }}</span>
              <span class="info-value">{{ ipInfo.country || '-' }} {{ ipInfo.countryCode ? `(${ipInfo.countryCode})` : '' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.province') }}</span>
              <span class="info-value">{{ ipInfo.regionName || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.city') }}</span>
              <span class="info-value">{{ ipInfo.city || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.postalCode') }}</span>
              <span class="info-value">{{ ipInfo.zip || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.coordinates') }}</span>
              <span class="info-value">{{ ipInfo.lat }}, {{ ipInfo.lon }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.timezone') }}</span>
        ...
        <el-card>
          <template #header>
            <div class="card-header">
              ...
              <span>{{ t('tool.ip-lookup.networkInfo') }}</span>
            </div>
          </template>
          <div class="info-grid">
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.isp') }}</span>
              <span class="info-value">{{ ipInfo.isp || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.organization') }}</span>
              <span class="info-value">{{ ipInfo.org || '-' }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">{{ t('tool.ip-lookup.asInfo') }}</span>
